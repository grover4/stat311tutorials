...
The variable `road` describes the condition of the road at the scene of the accident.  Create a frequency table of `road` in descending order of frequency. See hints for help. Are the results surprising?


```{r road-frequency,exercise=TRUE}

```


```{r road-frequency-hint-1}
...
```{r road-frequency-hint-2}
accidents %>% 
  ___ %>% (___, sort=TRUE)
```

```{r road-frequency-hint-3}
accidents %>%
...
Suppose we wish to compare accident severity (`severity_desc`) by the condition of the road (`road`). Recreate the following segmented bar plot with the values of `road` on the vertical axis. Note that the rows that are missing the road conditions are not represented in the bar plot. This is achieved by filtering the data using the `is.na` function with `road` as the argument.    

```{r example-plot, fig.width=6,fig.height=4, echo=FALSE}

ggplot(data=accidents %>% filter(!is.na(road)), 
  mapping=aes(y=road, fill=severity_desc)) +
      geom_bar(position = "fill")+
      labs(x="Proportion", y="Road condition",
           fill="Severity Description",
           title="Road conditions and severity of accident")
...
# - filter the data using !is.na(road) 
# - map road to the y-axis and severity desc to the fill 
