...
Fit a linear model predicting total daily bike rentals `cnt` from raw daily temperature, use `tidy`to print the output and answer the questions below.
...
rm_season <- lm(___ ~ ___, data = ___)
glance(___)$___
```

```{r remove-season-hint-1}
rm_season <- lm(cnt ~ ___, data = ___)
glance(___)$adj.r.squared
```
...
rm_holiday <- lm(___ ~ ___, data = ___)
...
rm_workingday<- lm(___ ~ ___, data = ___)
glance(___)$___
```
...
rm_weathersit<- lm(___ ~ ___, data = ___)
glance(___)$___
```
...
rm_temp_raw<- lm(___ ~ ___, data = ___)
glance(___)$___
```
...
rm_feeltemp_raw <- lm(___ ~ ___, data = ___)
glance(___)$___
```
...
rm_humidity_raw <- lm(___ ~ ___, data = ___)
glance(___)$___
```
...
rm_windspeed_raw <- lm(___ ~ ___, data = ___)
glance(___)$___
```
...
rm_interaction <- lm(___ ~ ___, data = ___)
glance(___)$___
```
...
This tutorial is exercpted from Data Science in a Box.