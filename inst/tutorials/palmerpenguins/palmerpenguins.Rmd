...
mass_flip_fit <- lm(body_mass_kg ~ flipper_length_cm, data = penguins)
```
...
mass_spec_fit <- lm(body_mass_kg ~ species, data = penguins)
```
...
mass_len_fit <- lm(body_mass_kg ~ bill_length_cm, data = penguins)
```
...
mass_dep_fit <- lm(body_mass_kg ~ bill_depth_cm, data = penguins)
```
...
We'll use the **tidyverse** and `broom` packages for this analysis and the **palmerpenguins** package for the data.
...
In the following code block, use `mutate()` to add the new variable `body_mass_kg` as that records body mass in **kilograms**.
...
The penguins' flipper lengths, bill lengths and bill depths are measured in millimeters.
...
Note that 1 millimeter = 0.1 centimeter.
