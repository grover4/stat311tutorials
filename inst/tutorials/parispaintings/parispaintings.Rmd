...
```{r plot-log-price-surface, exercise=TRUE, exercise.setup="log-price-surface-setup" }
...
Instead of manually calculating these numbers, we can simply use the function `lm`. The linear model function is specified using a formula syntax $y \~ x$.  The resulting estimates may be viewed in a tidy format by inputting the result from `lm` into `tidy`. 

```{r model-fit,exercise=TRUE, exercise.setup="log-price-surface-setup"}
...
```{r model-fit-check}
grade_this({
  if(identical(round(.result$estimate[1], digits=1), 5.6)) {
...
question("Which interpretations are correct?",
  ...
  answer("When the surface area increases by a factor of 10, the price increases on average by a factor of 15% ",
...
**Hint:**
 - First we need convert a surface area of 100 to the corresponding value for `log_surface`
...
```{r augment-predict, exercise = TRUE, exercise.setup = "log-price-surface-setup"}
...
```{r newprice, exercise = TRUE, exercise.setup="log-price-surface-setup"}
...
```{r predict-newprice, exercise = TRUE, exercise.setup="log-price-surface-setup"}
___(___)
```
