...
In addition, we will make use of the `skim` function from the **skimr** package to get a quick vbroad overview of the data frame. Run the following code to load these packages.
...
```{r plot-price-surface-hint-4, fig.width=7, fig.height=3}
...
Let's address the last issue first by reviewing a quick summary of the variables `price` and `Surface` using the `skim` function. 
...
Also print summaries of your new variables using `skim` as we did in the previous code chunk.
...
Let's interpret the slope coefficient for `log_surface`. Recall that the slope is the change in the response associated with a unit change in the explanatory variable. When variables are transformed, as they are here, we need to carefully examine what this translates to on the original untransformed scale. 

Let's denote the slope coefficient for `log_surface` by $b$ 
...
$$log(\mbox{price for surface 2x}) - log(\mbox{price for surface x}) = b$$
$$log\left( \frac{\mbox{price for surface 2x}}{\mbox{price for surface x}} \right) = b$$
$$2^{log\left( \frac{\mbox{price for surface 2x}}{\mbox{price for surface x}} \right) } = 2^b$$
$$\left( \frac{\mbox{price for surface 2x}}{\mbox{price for surface x}} \right) = 2^{b}$$

Now use the numerical value for the slope coefficient, $b$, from the output of `tidy` and  answer the following questions:
