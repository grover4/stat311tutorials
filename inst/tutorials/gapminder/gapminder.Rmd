...
We are going to focus on the life expectancy in the state of Washington which happens to have 39 counties. Filter the `life` dataset for counties from "Washington", and name the resulting data frame `life_wa`. Which county in WA state has the highest life expectancy? the lowest? 
...
life <- life %>%
  mutate(coast = ifelse( ___ %in% c("___", "___", "___"), "___", "___") ) 
```
...
Let's compare the distributions of  `income` on the West Coast and in the rest of the country to see what shape these distributions take. There are several plot types that we could use here. We will construct density ridge plots. The dataset `life` with the variable `coast` that you created earlier is still in your workspace. 


To make ridge plots, use `geom_density_ridges` and map the `y` aesthetic to the categorical variable. You can, in addition map `fill` to the same categorical variable in order to vary the color of each ridge plot. We will also specify an `alpha` level of 0.3. This allows the colors to be somewhat transparent so that we can see where they overlap.
...
```{r inc, exercise = TRUE, exercise.setup = "coast"}
...
```{r inc-hint-2}
ggplot(data = life,
       mapping = aes( x = income, y = coast, fill = coast))+
  geom___() 
...
We saw some extreme values when we created the density plot of income for counties on the West Coast. What are we to make of this blip of counties? One thing we can do is also display the data using a boxplot. 

```{r box-income, exercise=TRUE, exercise.setup = "coast"}


```


```{r box-income-solution}
...
```{r inc2, exercise=TRUE, exercise.setup="coast"}
