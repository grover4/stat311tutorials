...
life_wa = life %>%
  ...
  mutate(expectancy = round(expectancy, digits = 1 )) %>%
...
We are going to focus on the life expectancy in the state of Washington which happens to have 39 counties. Let's filter the data for counties from "Washington", and name the resulting data frame `life_wa`. To make the rest of the conversation simpler we'll also round the life expectancy values to one decimal place. Finally just select the variables `county` and `expectancy`.
...
life_wa <- life %>%
  filter(state == "___") %>%
  mutate(expectancy = round(expectancy, digits = ___ )) %>%
  select(___, ___)
...
Let's take a closer look at the life expectancies for counties in Washington. We'll arrange them in ascending order to make it a bit easier to to answer the question "What is a typical value for life expectancies in WA counties?"
...
To answer this question, we need to think about what "typical" means. One statistic we commonly use to describe a typical observation is the **mean**, or in other words, the arithmetic average. 
...
The value that cuts the data in half is `r life_wa_summary$med`. We can also calculate this using the `median()` function.
...
The mean can be thought of as the balance point of the data. This highlights an important feature of the mean: its sensitivity to extreme values. For this reason, when working with skewed distributions, the median is often a more appropriate measure of center.
...
question("A track coach wants to determine an appropriate heart rate for an average athlete during their workouts. She chooses five of her runners and asks them to wear heart monitors. In the middle of the workout, she reads the following heart rates for the five athletes: 130, 135, 140, 145, 325. Which of the following is an appropriate measure of center for this data?",
  ...
  answer("Median", correct = TRUE, message = "Nice! "),
...
Suppose we are interested in comparing life expectancy in the states California, Oregon, Washington with the rest of the country. To make this comparison, we start by creating a new variable called `coast` that takes the value "west" if the state value is one of "California", "Oregon", or "Washington", and "not west" otherwise, and save it back to the original dataset. We then `group_by` this new variable and calculate summaries of life expectancy. 


Fill in the blanks in the following code to calculate the mean and median life expectancy by coast.

```{r life-groupby, exercise=TRUE}
life %>%
  mutate(coast = ifelse( state %in% c("___", "___", "___"), "___", "not west") ) %>%
  group_by(___) %>%
  summarize( mean = mean(___), median = median(___) )
```

```{r life-groupby-hint-1}
life %>%
  mutate(coast = ifelse( state %in% c("Washington", "Oregon", "California"), "west", "not west") ) %>%
  group_by(___) %>%
  summarize( mean = mean(___), median = median(___) )
```
...
We learn that looking at both mean and median, that the typical West Coast county has a slightly higher average life expectancy than counties not on the West Coast.

`group_by()` and `summarize()` form a powerful pair of functions, so let's look into how they work. 
When we add a line to group by `coast`, we are effectively breaking the dataset into two groups and calculating the `mean()` or `median()` of the expectancy variable for each group separately.

```{r life-gr-sum-gt}
life <- life %>%
  mutate(coast = ifelse( state %in% c("California", "Oregon", "Washington"), "west", "not west") )
...
The functions `group_by()` and `summarize()` open up lots of possibilities for analysis, so let's get started.
...
Throughout this lesson, you will use data from `gapminder`, which tracks demographic data in countries of the world over time. Take a peek at the dataframe in the code chunk below.
...
gap2007 <- gapminder %>% filter(___)

gap2007 %>%
  group_by(___) %>%
  summarize(mean = ___, median = ___)
...
ggplot(data = gap2007, mapping = aes(x = continent, y = lifeExp)) +
 geom_boxplot() 
```
...
Let's consider the life expectancies in Washington again. We think about variability in a dataset as the spread of the data around its center. 

```{r ref.label = "life-wa-plot"}
```
...
```{r wa-deviation-sq-mean-samp, echo = TRUE}
life_wa %>%
  mutate(
    deviation = expectancy - life_wa_summary$mean,
    deviation_sq = deviation^2
...
By comparison, the variance of this sample is `r life_wa_summary$var` years squared, which is a unit that we have no real intuition about.
...
Let's say that King County, the county with a life expectancy around 81, instead had a life expectancy of 97. We'll create a new dataset with that value and call it `life_wa_extreme`.

```{r life-ma-extreme, echo = TRUE}
life_wa_extreme <- life_wa %>%
  mutate(expectancy = if_else(county == "King County", 97, expectancy))
```

Below is a look at the dot plot of the data with the made up extreme value.
...
- For each continent in `gap2007`, summarize life expectancies using the `sd()`, the `IQR()`, and the count of countries in each continent using the `n()` function. Keep in mind that `n()` does not take any arguments!

- Graphically compare the spread of these distributions by constructing density plots of life expectancy faceted by continent.
...
```{r ex2-hint-4}
## Last step -- create density plots with the different continents # faceted by continent.  We will set the argument scales="free" rather than "fixed" so we can properly view the shape of each distribution

ggplot(data = gap2007, mapping = aes(x = lifeExp) ) +
  geom_density() + 
...
Using the shapes of the density plots you created above, calculate the most appropriate measures of center and spread for the distribution of life expectancy in the countries in Asia. Note you'll need to apply a filter here!
...
```{r ex3-hint-1}
# First filter the data to only include Asia
gap2007 %>%
...
```{r ex4, exercise = TRUE, exercise.setup="ex2-setup"}
...
when conducting an exploratory data analysis. A fourth and final thing to look for are outliers. These are observations that have extreme values far from the bulk of the distribution. They're often very interesting cases, but they're also good to know about before proceeding with more formal analysis.

We saw some extreme values when we created the density plot of income for counties on the West Coast. What are we to make of this blip of counties? One thing we can do is display the data using a boxplot. To make the boxplot a bit easier to compare with a density plot, you can specify the categorical variable (`coast`) as the y-variable instead of a `fill`. This option labels the y axis with the values of `coast` rather than in a separate legend. 

```{r, echo = TRUE, eval = FALSE}
ggplot(life, aes(x = income, fill = coast)) +
  geom_density(alpha = 0.5)

ggplot(life, aes(x = income, y = coast)) +
  geom_boxplot(alpha = 0.5) 
```


```{r coast-income, fig.width = 8}
p1 <- ggplot(life, aes(x = income, fill = coast)) +
  geom_density(alpha = 0.5)

p2 <- ggplot(life, aes(x = income, y = coast)) +
  geom_boxplot(alpha = 0.5) 
p1 + p2
```

What we see is interesting: the boxplot flags many counties as outliers, both along the West Coast but also in the rest of the country as well. So why was the blip more apparent on the West Coast? It has to do with sample size. There are far fewer counties in the West Coast group, so these few outliers have an over-sized effect on the density plot. In the case of the non-West Coast group, there are many more counties that ended up washing out the effect of the outliers in the density plot.
...
A useful tool when inspecting outliers is to filter the dataset to only include outliers, and then arrange them in decreasing order. Let's do that next.
...
ggplot(data = life %>% filter(!is_outlier), mapping = aes(x = income, fill = coast)) + 
  geom_density(alpha = 0.3)
...
Next, we'll build a plot with that country removed. The `gap2007` is still available in your workspace. To carry out this process we need to use the following steps: 
 ...
 - Use `filter()` to retain only observations from Asia

 - Use `mutate()` to create a new variable called `is_outlier`

 - Use `if_else()` to specify how the `is_outlier` variable should be made, where countries with a life expectancy less than 50 should get a value of `TRUE` and everyone else should get a value of `FALSE`

 - Create a boxplot of the life expectancies of the non-outlier countries



```{r ex5, exercise = TRUE, exercise.setup="ex2-setup"}
...
```{r ex5-solution}
# Create a boxplot of the distribution of life expectancy with the outlier removed by piping in the dataframe to ggplot
gap2007 %>%
  filter(continent == "Asia") %>%
  mutate(is_outlier = if_else(lifeExp < 50, TRUE, FALSE)) %>% 
  filter(!is_outlier) %>%
