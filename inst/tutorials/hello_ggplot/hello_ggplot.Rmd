...
2. Make a histogram of the `Sepal.Width`. Try binwidths of 0.1, and 0.3. Don't forget to label the axes and come up with a title. (Don't forget, R is case sensitive)

```{r sepal-width-histo, exercise=TRUE}


```

```{r sepal-width-histo-solution}

ggplot(data=iris, mapping=aes(x=Sepal.Width) )+
  geom_histogram(binwidth=0.3)+
  labs(x="Sepal Width (cm)", 
       y=Frequency,
       title="Sepal widths of iris",
       caption="Source: iris data frame")
```


```{r sepal-width-dist-bin-quiz}
question("Which binwidth is more suitable?", 
  answer("0.1",
         message = "That seems a bit narrow. The histogram is a little too jagged."),
  answer("0.3",
         correct = TRUE,
         message = "That's a good width - certainly better than 0.1."),
  answer("Neither are at all suitable."),
  allow_retry = TRUE
)
```

3.Make a boxplot showing the `Petal.Width` by `Species`. 

```{r petal-width-species-box, exercise=TRUE}


```
```{r petal-width-species-quiz}
question("Which species has the largest flowers?", 
  answer("Setosa"),
  answer("Versicolor"),
...
```{r petal-width-species-box-solution}
ggplot(data=iris, mapping=aes(x=Petal.Width,fill=Species))+
  geom_boxplot()
```


3.Make a scatterplot of the `Sepal.Length` against `Sepal.Width`. Allow the color of the points to vary by Species. Also draw a smooth trend line. Vary the color of the points (but not the trend line) by `Species`.

```{r sepal-length-width, exercise=TRUE, exercise.lines=5}
##
##
```
