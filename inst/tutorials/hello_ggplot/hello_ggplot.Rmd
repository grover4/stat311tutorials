...
**ggplot2** is tidyverse's data visualization package - The `gg` in "ggplot2" stands for Grammar of Graphics because the package is inspired by a similarly titled book written by Leland Wilkinson. The book develops a grammar for graphics that enable us to concisely describe the components of a graphic. 
...
In both the scatter plots of the previous section , one group of cars (circled below) have a higher mileage than you might expect for the size of their engine. 
...
#ggplot(data = mpg,
#      mapping = aes(x = displ, y = hwy, ___________)) + 
#  geom_point()+
...
The use of an aesthetic like color helps bring a third variable in view when the `geom` is a point as with a scatter plot. If the `geom` were a boxplot or a histogram, the color aesthetic would simply apply to the outline of the shape. To fill in the shape with color, we would need to use `fill` rather than `color`. 
...
ggplot2 added a default scale for each aesthetic. 
...
For example, the following code produces a density plot of highway fuel efficiency categorized by the drive train. The bandwidth (similar to binwidth in histograms) is chosen to be twice the default value by setting `adjust=2` and `alpha`, the transparency parameter is set to 0.5.  
...
```{r penguin-scatter, exercise=TRUE}


```

```{r penguin-scatter-hint-1}
...
Finally, thanks are always due to Mine &Ccediletinkaya-Rundel for her wonderful Data Science in a Box course materials 
