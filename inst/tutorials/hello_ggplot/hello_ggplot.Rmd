...
To use ggplot2 functions, we need to begin by loading **tidyverse** by running the following code.
...
### Creating a ggplot
 ...
 The plot shows a negative relationship between engine size (`displ`) and highway fuel efficiency (`hwy`) as one might expect. Does this match up with your intuition from earlier?
 
 Modify the code in the previous chunk to produce a scatter plot of the relationship between gas mileage for city driving (`cty`) and engine size. 
...
### Recap 
 ...
 With ggplot2, you begin a plot with the function `ggplot()`. This creates a blank coordinate system. To this you add a layer of points using `geom_point()`. `ggplot()` comes with many geom functions that each add a different layer to a plot. You can actually use multiple geoms in a single ggplot.
...
`r fontawesome::fa("question-circle")` What happens if you map an aesthetic (say color) to `color=(displ < 5)`? Take a guess and then see if you are right. 
...
In the above example we mapped the color aesthetic to the `class` variable from the dataframe. This only makes sense to do since `class` takes a handful of distinct values.
...
A `geom` is a geometric object which represents the data. To put it simply, `geom` tells ggplot what type of graph we want. In fact, we can overlay a smooth trend line to our scatterplot by adding a `geom_smooth` layer to `ggplot()`.

```{r ex6, echo=T,message=F}
...
If we place any mappings in a geom layer, ggplot2 will treat them as local mappings for that layer only. For example, the following code will allow the color of the trend line to vary by the type of drive train. 

```{r ex7, echo=T,message=F}
ggplot(data=mpg,mapping=aes(x=displ,y=hwy)) + 
   geom_point() +
...
Now that you have a few tricks under your belt, let’s use them on a different dataframe. The `iris` data is a preloaded dataset that comes with R. It gives measurements taken on 50 flowers from each of 3 species of iris.
...
2.Make a scatterplot showing the sepal length against width. Allow the color of the points to vary by Species. Also draw a smooth trend line. Vary the color of the trend line by `Species`.
