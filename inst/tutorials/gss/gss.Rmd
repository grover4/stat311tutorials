...
In the following exercise, we'll be using the `infer` package to bootstrap confidence intervals.


What we want to do now is calculate a 95% bootstrap confidence interval for the mean amount of time that Americans spend on email weekly.
...
- `specify()` is used to specify which columns in our data frame are  the relevant response (and if applicable, explanatory) variables. It is enough to specify just the response for a point estimate on one variable, as here.

- `generate()` generates the bootstrap samples.

- And finally, `calculate()` calculates the sample statistic. In this case, we're using the mean, but we could just as easily calculate, say, medians.


Fill in the blanks in the following code block to create a bootstrapped data frame which contains the mean amount of time that individuals in each of 1,000 bootstrap samples spend checking email at work:
...
```{r glimpse-boot-df, exercise = TRUE, exercise.setup = "setup-boot-df-mean"}
boot_df
```
...
We can easily plot the distribution of `stat` using `ggplot`. Go ahead and make a histogram of the bootstrapped means in the `boot_df` dataframe. Play around the binwidth to create a smooth picture.

```{r visualize_stat, exercise=TRUE, exercise.setup="setup-boot-df-mean"}
...
Now we have the `boot_df` data frame, we can construct the confidence interval itself by picking out a certain percentage of the center of the distribution of the variable `stat`.
Since we're after a 95% confidence interval, we take the center 95% of the bootstrap distribution, and find the range of these values.
...
```{r confidence-interval-email, exercise = TRUE, exercise.setup = "setup-boot-df-mean"}
___ %>%
...
grade_this({
  if(identical(floor(as.numeric(.result$lower[1])), 385) & identical(floor(as.numeric(.result$upper[1])), 452)) {
    pass("That is the 95% confidence interval for mean weekly time spent on email.")
  }
...
```{r confidence-99-email, exercise = TRUE, exercise.setup = "setup-boot-df-mean"}
boot_df %>% 
...
Let's practice calculating one more confidence interval. Go ahead and calculate the 90% confidence interval for the mean amount of time Americans spend on email 
weekly. 

```{r confidence-90-email, exercise = TRUE, exercise.setup = "setup-boot-df-mean"}
boot_df %>% 
   summarise( lower = ___(___, ___), 
              upper = ___(___, ___ ))
```
...
And finally, we want to construct a 90% confidence interval for the *median* amount of time Americans spend on email weekly. 
...
boot_df_median %>% ___( lower = ___(___, ___), 
                        upper = ___(___, ___) )
```
