...
Give this a try. Delete the underline in the code below and write `class` in its place. Run the code after removing the comment (`#`) symbols. What happens?

```{r color-class, exercise = T}
ggplot(data = mpg, mapping = aes(x = displ, y = hwy, color = ___ )) + 
  geom_point() + 
...
ggplot(data = mpg, mapping = aes(x = displ, fill = trans) )+
  geom_boxplot()
```
...
```{r encode, echo=FALSE}
learnrhash::encoder_ui(ui_before=NULL, ui_after=default_ui(url="https://canvas.uw.edu/courses/1483091") )
```
